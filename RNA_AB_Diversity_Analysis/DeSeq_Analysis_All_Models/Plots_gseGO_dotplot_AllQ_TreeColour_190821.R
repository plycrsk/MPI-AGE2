## Plots for gseGO analysis at all Q values ## 

library("ggplot2")
library("dplyr")
library("stringr")
library("forcats")

ego_summary <- read.csv("Results/GSEA_Analysis/gsea_simplified_results_Q0.00_WithGOTree_190821.csv")
ego_summary <- ego_summary[order(-abs(ego_summary$NES)),]
ego_summary$Q = "0.00"
ego_summary$type[ego_summary$NES < 0] = "Downregulated"
ego_summary$type[ego_summary$NES > 0] = "Upregulated"
#dot_df = ego_summary
dot_df = ego_summary[1:15,] ## small dataset



ego_summary2 <- read.csv("Results/GSEA_Analysis/gsea_simplified_results_Q1.00_WithGOTree_190821.csv")
ego_summary2 <- ego_summary2[order(-abs(ego_summary2$NES)),]
ego_summary2$Q = "1.00"
ego_summary2$type[ego_summary2$NES < 0] = "Downregulated"
ego_summary2$type[ego_summary2$NES > 0] = "Upregulated"
#dot_df2 = ego_summary2
dot_df2 = ego_summary2[1:15,] ## small dataset

ego_summary3 <- read.csv("Results/GSEA_Analysis/gsea_simplified_results_Q1.50_WithGOTree_190821.csv")
ego_summary3 <- ego_summary3[order(-abs(ego_summary3$NES)),]
ego_summary3$Q = "1.50"
ego_summary3$type[ego_summary3$NES < 0] = "Downregulated"
ego_summary3$type[ego_summary3$NES > 0] = "Upregulated"
#dot_df3 = ego_summary3
dot_df3 = ego_summary3[1:15,] ## small dataset

ego_summary4 <- read.csv("Results/GSEA_Analysis/gsea_simplified_results_Q2.00_WithGOTree_190821.csv")
ego_summary4 <- ego_summary4[order(-abs(ego_summary4$NES)),]
ego_summary4$Q = "2.00"
ego_summary4$type[ego_summary4$NES < 0] = "Downregulated"
ego_summary4$type[ego_summary4$NES > 0] = "Upregulated"
#dot_df4 = ego_summary4
dot_df4 = ego_summary4[1:15,] ## small dataset

ego_summary5 <- read.csv("Results/GSEA_Analysis/gsea_simplified_results_Q3.00_WithGOTree_190821.csv")
ego_summary5 <- ego_summary5[order(-abs(ego_summary5$NES)),]
ego_summary5$Q = "3.00"
ego_summary5$type[ego_summary5$NES < 0] = "Downregulated"
ego_summary5$type[ego_summary5$NES > 0] = "Upregulated"
#dot_df5 = ego_summary5
dot_df5 = ego_summary5[1:15,] ## small dataset

ego_summary6 <- read.csv("Results/GSEA_Analysis/gsea_simplified_results_Q4.00_WithGOTree_190821.csv")
ego_summary6 <- ego_summary6[order(-abs(ego_summary6$NES)),]
ego_summary6$Q = "4.00"
ego_summary6$type[ego_summary6$NES < 0] = "Downregulated"
ego_summary6$type[ego_summary6$NES > 0] = "Upregulated"
#dot_df6 = ego_summary6
dot_df6 = ego_summary6[1:15,] ## small dataset

dot_df <- rbind(dot_df, dot_df2, dot_df3, dot_df4, dot_df5, dot_df6)

dot_df$ID <- paste(dot_df$ID,dot_df$Q)

gene_count <- dot_df %>% group_by(ID) %>% summarise(count = sum(str_count(core_enrichment, "/"))+ 1)
dot_df <- left_join(dot_df, gene_count, by = "ID") %>% mutate(GeneRatio = count/setSize)

dot_df <- dot_df[order(dot_df['Description']),]

immune_terms <- dot_df[dot_df$Immune_System_Processes == TRUE,]
#other_terms <-dot_df[dot_df$Immune_System_Processes == FALSE,]

X11()


## from Tommy's code
p <- ggplot(dot_df, aes(x = enrichmentScore, y = fct_reorder(Description, -p.adjust))) + 
  #geom_point(aes(size = count, color = p.adjust)) +
  geom_point(aes(color=type, size = 8)) +
  theme_bw(base_size = 11) +
  #scale_colour_gradient(limits=c(0, 0.10), low="red") +
  geom_point(data = immune_terms, aes(x = enrichmentScore, y = fct_reorder(Description, -p.adjust))
             ,color="black", size = 5, pch=21) +
  ylab(NULL)

p + facet_grid(.~Q)

dev.copy2pdf(file = "gseGO_dotplot_QALL_coloured_top15_direction_Simplified_190821.pdf")

ggsave(
  "gseGO_dotplot_QALL_coloured_top15_direction_Simplified_190821.png",
  plot = last_plot(),
  device = NULL,
  path = NULL,
  scale = 1,
  width = 14,
  height = 6.99,
  units = c("in", "cm", "mm", "px"),
  dpi = 300,
  limitsize = TRUE,
  bg = NULL
)
