...
ego_summary <- read.csv("Results/RemovingFish/GSEA/sea_simplified_results_Q0.00_WithGOTreeFinal_NoFish4_230821.csv")
...
ego_summary2 <- read.csv("Results/RemovingFish/GSEA/sea_simplified_results_Q1.00_WithGOTreeFinal_NoFish4_230821.csv")
...
ego_summary3 <- read.csv("Results/RemovingFish/GSEA/sea_simplified_results_Q1.50_WithGOTreeFinal_NoFish4_230821.csv")
...
ego_summary4 <- read.csv("Results/RemovingFish/GSEA/sea_simplified_results_Q2.00_WithGOTreeFinal_NoFish4_230821.csv")
...
ego_summary5 <- read.csv("Results/RemovingFish/GSEA/sea_simplified_results_Q3.00_WithGOTreeFinal_NoFish4_230821.csv")
...
ego_summary6 <- read.csv("Results/RemovingFish/GSEA/sea_simplified_results_Q4.00_WithGOTreeFinal_NoFish4_230821.csv")
...
#need to add colour for all immune go terms
immune_terms_up <- subset(dot_df_up, immune_system_processes == TRUE | inflammatory_response == TRUE
                         | regulation_of_immune_system_processes == TRUE | regulation_of_inflammatory_response == TRUE)

immune_terms_down <- subset(dot_df_down, immune_system_processes == TRUE | inflammatory_response == TRUE
                            | regulation_of_immune_system_processes == TRUE | regulation_of_inflammatory_response == TRUE)
#other_terms <-dot_df[dot_df$Immune_System_Processes == FALSE,]
...
p <- ggplot(dot_df_up, aes(x = NES, y = fct_reorder(Description, NES))) + 
  ...
  geom_point(data = immune_terms_up, aes(x = NES, y = fct_reorder(Description, NES))
             ,color="yellow", size = 5) +
  ylab(NULL)

p + facet_grid(.~Q)

dev.copy2pdf(file = "gseGO_dotplot_QALL_coloured_top15_upregulated_Simplified_NoFish4_220821.pdf")

ggsave(
  "gseGO_dotplot_QALL_coloured_top15_upregulated_Simplified_NoFish4_220821.png",
  plot = last_plot(),
  device = NULL,
  path = NULL,
  scale = 1,
...
p <- ggplot(dot_df_down, aes(x = NES, y = fct_reorder(Description, NES))) + 
  ...
  theme_bw(base_size = 11) +
  #scale_colour_gradient(limits=c(0, 0.10), low="red") +
  geom_point(data = immune_terms_down, aes(x = NES, y = fct_reorder(Description, NES))
             ,color="yellow", size = 5) +
  ylab(NULL)
...
dev.copy2pdf(file = "gseGO_dotplot_QALL_coloured_top15_downregulated_Simplified_NoFish4_220821.pdf")

ggsave(
  "gseGO_dotplot_QALL_coloured_top15_downregulated_Simplified_NoFish4_220821.png",
  plot = last_plot(),
  device = NULL,
  path = NULL,
  scale = 1,
